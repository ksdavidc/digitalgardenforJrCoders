---
{"dg-publish":true,"permalink":"/digital-garden/torchlight/torchlight-class-1-japanese/"}
---
...
最初のスプライトを作成します。それを __Hitbox__ と呼んでください__[^1]
- 重要: __hitbox__ はただの正方形 です。後で本物の衣装にしますが、今は四角いだけです。.
...
- 初期化は通常、__position__、__direction__、および __effects__ などを意味します。また、すべての __variables__ を初期化します。
...
- 最初はすべての変数を知っているとは限りませんが、これはプラットフォーマーなので、「`x 速度`」と「`y 速度`」が必要になることがわかっているので、それらを初期化しましょう。
...
- __position__ を設定するには、次のトリックを使用します: スプライトをステージの開始位置に配置します。次に、ブロック領域で「x に移動」ブロックを見つけます。スクラッチはブロックに正しい数字を入れます。
